...
public class BarChartRenderer extends BarLineScatterCandleBubbleRenderer {
    ...
    public boolean isDrawRoundRect = true;  //判断用户是否需要绘制上下为圆角的柱子

    public BarChartRenderer(BarDataProvider chart, ChartAnimator animator,
                            ViewPortHandler viewPortHandler,boolean isDrawRoundRect) {
    ...
    protected void drawDataSet(Canvas c, IBarDataSet dataSet, int index) {
        ...
        for (int j = 0, pos = 0; j < buffer.size(); j += 4, pos++) {
            ...
            if (isCustomFill) {
                ...
                Log.e("MichalTest","left="+buffer.buffer[j]+";top="+buffer.buffer[j+1]+";right="+buffer.buffer[j+2]+";bottom="+buffer.buffer[j+3]);
