...
public class MonthView extends LinearLayout {
     ...
     * 设置自定义title view
     *
     * @param ViewNumber common title bar view
     */
    public void setCustomTitleView(int ViewNumber) {
        View customTitleView = null;
        if (ViewNumber == 0){
            customTitleView = getDefaultTitleView();
        }else if (ViewNumber == 1){
            customTitleView = getRoxTitleView();
        }
        if (getChildCount() > 5)
            removeViewAt(0);
    ...
    private View getRoxTitleView(){
        View view = View.inflate(getContext(),R.layout.calendar_title_layout,null);
        return view;
    }
